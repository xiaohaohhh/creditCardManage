...
- **共享额度账户**：支持多张卡片挂到同一个共享额度账户下，总额度按共享账户去重统计。
...
- **云端备份与恢复**：单向备份卡片到云端服务器，支持从云端恢复数据。
- **本地 JSON 导入导出**：支持将本地数据库导出为 JSON 文件，并可在导入前自动备份当前数据。
- **邮件账单自动获取**：通过 IMAP 协议连接邮箱，自动拉取银行账单邮件，解析并匹配到已有卡片。
- **归属人分类**：支持按归属人（本人、配偶、父母等）筛选卡片。
...
应用在 **设置 → 本地数据导入导出** 中提供本地备份能力，适合迁移设备、手动备份或恢复数据。

### 导出为 JSON

1. 打开 **设置** 页面。
2. 点击 **导出为 JSON**。
3. 浏览器会下载一个类似 `credit-card-manager-export-YYYY-MM-DD_HH-mm-ss.json` 的文件。

导出内容包括：
- IndexedDB 中的全部表数据（当前包括共享账户、卡片和设置）
- 本地同步配置：`serverUrl`、`deviceId`、`lastSyncAt`

### 从 JSON 导入

1. 打开 **设置** 页面。
2. 点击 **从 JSON 导入** 并选择备份文件。
3. 确认导入后，应用会先自动下载一份当前本地数据备份：
   `credit-card-manager-pre-import-backup-YYYY-MM-DD_HH-mm-ss.json`
4. 随后才会覆盖当前本地数据库和同步配置。
5. 导入成功后会自动返回首页，并显示“导入完成，卡片列表已刷新”的提示。

### 使用注意

- JSON 文件可能包含敏感信息（如卡片数据、同步配置），请妥善保管。
- 导入会覆盖当前本地数据，因此建议保留最近一次导出文件。
- 导入会恢复备份文件中的 `deviceId`、`serverUrl` 和 `lastSyncAt`。
- 如果浏览器拦截自动下载，请允许下载，否则导入前自动备份可能无法保存到本地。
